  ...
  const renderDoctorCard = ({ item }: { item: any }) => (
    ...
    >
      ...
      <View className="flex-row items-center gap-1 mt-0.5">
        ...
        <Text className="text-xs text-slate-500 text-center" numberOfLines={1}>{item.hospital}</Text>
      </View>
    ...
    <View className="flex-1 bg-slate-50">
      ...
      {doctors.length === 0 ? (
        <View className="flex-1 justify-center items-center p-8">
          ...
          <Text className="text-sm text-slate-500 text-center">No doctors are currently available in this department</Text>
        </View>
      ) : (
        <FlatList
          data={doctors}
